...
let detecting = false;
let detectionDelay = 1; // Delay in milliseconds between detections
...
function setup() {
  // create canvas element with 640 width and 480 height in pixels
...
// callback function. it is called when an object is detected
function onDetected(error, results) {
  if (error) {
    console.error(error);
  }
  detections = results;
  // keep detecting objects with a delay
  if (detecting) {
    setTimeout(detect, detectionDelay);
  }
}

function detect() {
  // instruct "detector" object to start detecting objects from the video element
  // and "onDetected" function is called when an object is detected
